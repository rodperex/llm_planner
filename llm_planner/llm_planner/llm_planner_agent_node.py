...
class LLMPlannerAgentNode(Node):
    ...
    def plan_task_callback(self, request, response):
        ...
        base_user_prompt = (
            f'goal: "{goal}"\ncontext: "{context}"{skills_block}{preconditions_block}{postconditions_block}{useful_info_block}\n\n'
    ...
    def replan_task_callback(self, request, response):
        ...
        useful_info = request.useful_info.strip() if hasattr(request, 'useful_info') else ''
        useful_info_block = f'USEFUL INFORMATION (from orchestrator):\n  ' + useful_info.replace('\n', '\n  ') + '\n\n' if useful_info else ''
        ...
        response.success      = True
        plan_yaml = f'\n\n# Replan generated by {self.llm_model_id}.' + '\n\n' + plan_yaml
